...
const CARGO_OPTIONS: Cargo[] = ['jogador', 'treinador_principal', 'treinador_assistente'];
      ...
      <div style={{ ...cardStyle, padding: 22, display: 'flex', gap: 18, alignItems: 'flex-start', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 220, display: 'flex', flexDirection: 'column', gap: 10 }}>
          {editingEquipeInfo ? (
            ...
            <>
              <div className={isAdmin ? 'hover-reveal' : undefined} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {isAdmin && (
                  <button
                    ...
                    style={{ background: 'none', border: 'none', color: 'var(--text-faint)', cursor: 'pointer', padding: 2, display: 'flex', opacity: 0, transition: 'opacity .12s ease' }}
                  >
                    <Pencil size={13} strokeWidth={1.75} />
                  </button>
                )}
              </div>
              <div style={{ fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.5 }}>{equipe.descricao || 'Sem descrição ainda.'}</div>
            </>
          )}
        </div>
